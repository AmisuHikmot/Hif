...
export function OrderSuccessClient({ reference }: OrderSuccessClientProps) {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        const response = await fetch(`/api/track-order?reference=${encodeURIComponent(reference)}`)
        ...
        setOrder(orderData)
        const orderItems = orderData.shop_order_items_enhanced || []
        setItems(orderItems)
        ...
        if (orderData.has_digital) {
          setDownloads(
            orderItems
              .filter((item: any) => item.download_token)
              .map((item: any) => ({
                id: item.id,
