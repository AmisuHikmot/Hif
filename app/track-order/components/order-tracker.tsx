...
export function OrderTracker({ order }: OrderTrackerProps) {
  const steps = useMemo(() => {
    const isDigitalOnly =
      order.has_digital &&
      order.shop_order_items_enhanced?.length > 0 &&
      order.shop_order_items_enhanced.every((item) => item.product_type === "digital")

    return isDigitalOnly
      ? [
          { id: "pending", label: "Order Placed", icon: ShoppingBag, subtitle: "We have received your order" },
          { id: "processing", label: "Payment Confirmed", icon: ShieldCheck, subtitle: "Your payment has been verified" },
          { id: "delivered", label: "Delivered", icon: PartyPopper, subtitle: "Your digital order is ready" },
        ]
      : STEPS
  }, [order.has_digital, order.shop_order_items_enhanced])

  const statusIndex = useMemo(() => {
    const statusMap = Object.fromEntries(steps.map((step, index) => [step.id, index]))
    return statusMap[order.order_status] ?? 0
  }, [order.order_status, steps])
        ...
        <div className="hidden md:block">
          <motion.div className="flex items-start justify-between" initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.12 } } }}>
            {steps.map((step, index) => {
              ...
              return <DesktopStep key={step.id} step={step as any} state={state} timestamp={timestamp} stepIndex={index} totalSteps={steps.length} />
            })}
          </motion.div>
        </div>
        <div className="space-y-0 md:hidden">
          <AnimatePresence>
            {steps.map((step, index) => {
              const state: StepState = index < statusIndex ? "completed" : index === statusIndex ? "active" : "pending"
              const timestamp = (order as any)[`${step.id}_at`] ?? null
              return <MobileStep key={step.id} step={step as any} state={state} timestamp={timestamp} stepIndex={index} totalSteps={steps.length} />
            })}
          </AnimatePresence>
        </div>
      </div>
