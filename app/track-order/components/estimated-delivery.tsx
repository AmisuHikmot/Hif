...
function buildWhatsAppLink(reference: string): string {
  const message = encodeURIComponent(
    `Assalamu Alaykum, my order ${reference} appears to be running late. Could you please provide an update? JazakAllahu Khairan.`
  )
  return `https://wa.me/+2349167656667?text=${message}`
}
  ...
  return (
    <div className="flex flex-col items-center">
      <motion.div
        ...
        className="min-w-[2.5rem] rounded-lg bg-blue-900/20 px-2.5 py-1.5 text-center"
      >
        <span className="text-lg font-bold tabular-nums text-blue-900">
          {String(value).padStart(2, "0")}
        </span>
      </motion.div>
      <span className="mt-1 text-[10px] uppercase tracking-wider text-blue-600/70">
        {label}
      </span>
    </div>
  )
          ...
          <div className="flex items-start gap-3 p-4">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-emerald-700">
                Your order was delivered on {formattedDate}.
                <br />
        ...
        <Card className="overflow-hidden border-amber-300 bg-amber-50">
          ...
          <div className="p-4">
            <p className="text-sm leading-relaxed text-amber-800">
              ...
              <span className="font-medium">{formattedDate}</span>. We apologise for the delay —
              our team is working to get your order to you as soon as possible insha&apos;Allah.
            </p>
            <button
              onClick={() => setShowSupport((s) => !s)}
              className="mt-3 text-sm font-medium text-amber-700 underline underline-offset-2 hover:text-amber-900"
            >
            ...
            >
              <Card className="overflow-hidden border-border">
                ...
                <div className="flex flex-col gap-2 p-4 sm:flex-row">
                  ...
                  >
                    <a href={buildWhatsAppLink(orderReference)} target="_blank" rel="noopener noreferrer">
                  ...
                  <Button asChild variant="outline" size="sm" className="flex-1 gap-2">
                    <a href={buildEmailLink(orderReference)}>
                      <Mail className="h-4 w-4" />
                      Email Support
                    </a>
                  </Button>
                  <Button asChild variant="outline" size="sm" className="flex-1 gap-2">
                    <a href="tel:+2349167656667">
      ...
      <Card className={`overflow-hidden border-blue-200 ${isUrgent ? "bg-blue-100" : "bg-blue-50"}`}>
        ...
        <div className="p-4">
          ...
          <p className="mt-4 text-xs text-blue-600/70">
            Delivery estimates are provided by HIF staff and may vary. Insha&apos;Allah your order
            will arrive on time.
          </p>
        </div>
      </Card>
