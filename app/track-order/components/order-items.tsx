...
function getDownloadStatus(item: TrackingItem, paymentStatus: string) {
  if (item.product_type !== "digital") {
    return { canDownload: false, label: "", reason: "not_digital" as const }
  }
  if (paymentStatus !== "paid") {
    return { canDownload: false, label: "Payment pending — download unavailable", reason: "payment_pending" as const }
  }
  if (!item.download_token || !item.expires_at) {
    return { canDownload: false, label: "Payment pending — download unavailable", reason: "payment_pending" as const }
  }
  if (new Date(item.expires_at) < new Date()) {
    return { canDownload: false, label: "Download link expired", reason: "expired" as const }
  }
  if (item.max_downloads !== null && item.download_count !== null && item.download_count >= item.max_downloads) {
    return { canDownload: false, label: `Download limit reached (${item.download_count}/${item.max_downloads})`, reason: "limit_reached" as const }
  }
  return { canDownload: true, label: "Download available", reason: "available" as const }
}
  ...
  if (reason === "expired") {
    return (
      <div className="space-y-1.5">
        ...
        <Button asChild variant="ghost" size="sm" className="h-auto w-full px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground">
          <a href={`mailto:support@hif.com.ng?subject=${encodeURIComponent("Download Extension Request")}&body=${encodeURIComponent(`Assalamu Alaykum,\n\nI would like to request a download extension for:\nProduct: ${item.product_name}\n\nJazakAllahu Khairan.`)}`}>
            Request extension →
          </a>
        </Button>
      </div>
  ...
  if (reason === "limit_reached") {
    return (
      <div className="space-y-1.5">
        ...
        <Button asChild variant="ghost" size="sm" className="h-auto w-full px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground">
          <a href={`mailto:support@hif.com.ng?subject=${encodeURIComponent("Download Limit Reset Request")}&body=${encodeURIComponent(`Assalamu Alaykum,\n\nI have reached the download limit for:\nProduct: ${item.product_name}\n\nCould you please assist?\n\nJazakAllahu Khairan.`)}`}>
            Contact support →
          </a>
        </Button>
      </div>
...
function OrderItemRow({ item, index, paymentStatus }: { item: TrackingItem; index: number; paymentStatus: string }) {
  ...
  return (
    <motion.div
      ...
      className={`rounded-xl border p-4 transition-colors ${isDigital ? "border-violet-200 bg-violet-50/40" : "border-border bg-background"}`}>
          ...
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-2">
                <Badge
                  variant="secondary"
                  className={`text-xs font-medium ${isDigital ? "border-violet-200 bg-violet-100 text-violet-700" : "border-blue-200 bg-blue-100 text-blue-700"}`} >
                  {isDigital ? (
      ...
      <CardContent className="p-4 sm:p-6">
        <AnimatePresence>
          {hasDigital && paymentStatus === "paid" && (
            <motion.div
              ...
              <p>
                Your digital {digitalItems.length === 1 ? "product is" : "products are"} ready to
                download. Links expire 72 hours after payment.
              </p>
            </motion.div>
          )}
        </AnimatePresence>
        <div className="space-y-3">
          {items.map((item, index) => (
            <OrderItemRow key={item.id ?? index} item={item} index={index} paymentStatus={paymentStatus} />
          ))}
        </div>
      </CardContent>
    </Card>
