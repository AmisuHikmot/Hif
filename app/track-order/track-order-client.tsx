...
import { Search, Mail, Package, AlertCircle, Wifi, WifiOff } from "lucide-react"
...
export function TrackOrderClient() {
    ...
    async (searchRef?: string) => {
      ...
      try {
        const res = await fetch(
          `/api/track-order?reference=${encodeURIComponent(refToSearch)}`
        )
  ...
  const handleSearchByEmail = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/track-order?email=${encodeURIComponent(trimmedEmail)}`
      )
      const data = await res.json()
  ...
  useEffect(() => {
    ...
    // Don't subscribe if order is in a terminal state
  ...
  // ── Polling fallback (60s) for non-Realtime envs ───────────
  ...
  return (
    <div className="space-y-6">
      {/* Realtime connection indicator — only shown when order is loaded */}
      <AnimatePresence>
      ...
      {/* Search form — hidden once an order is displayed */}
      <AnimatePresence mode="wait">
        {!order && (
          ...
          >
                    ...
                    <div className="space-y-5">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          className={
                            referenceError ? "border-destructive focus-visible:ring-destructive" : ""
                          }
            ...
            {/* Loading skeleton — shown below form during fetch */}
            <AnimatePresence>
              {isLoading && (
                <motion.div
                  initial={{ opacity: 0 }}
