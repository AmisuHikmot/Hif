...
export async function sendEmail(
  ...
  try {
    ...
    const result = await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL || "orders@hif.com.ng",
      to,
      subject,
      html: htmlContent,
    })
...
export async function sendMembershipConfirmationEmail(
  ...
  return sendEmail(
    email,
    `HIF Membership Confirmed - ${membershipId}`,
    `
      <div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;color:#1f2937">
        <div style="background:#ecfdf5;border-radius:8px;padding:24px;text-align:center">
          <h1 style="color:#064e3b;margin:0">Membership Confirmed</h1>
          <p style="margin:8px 0 0">Hamduk Islamic Foundation</p>
        </div>
        <div style="padding:24px 0">
          <p>Assalamu alaykum ${name},</p>
          <p>Your membership payment has been received and your HIF membership is now active.</p>
          <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:16px;margin:20px 0">
            <p><strong>Membership ID:</strong> ${membershipId}</p>
            <p><strong>Membership Type:</strong> ${membershipType}</p>
            <p><strong>Valid Until:</strong> ${new Date(expiresAt).toLocaleDateString("en-NG")}</p>
          </div>
          <p>Please keep your membership ID for renewals and membership support.</p>
